...
public class NotesList extends ListActivity {
   ...
   //The columns needed by the cursor adapter
    private static final String[] PROJECTION = new String[] {
            ...
            //NotePad.Notes.COLUMN_NAME_BACK_COLOR,//扩展显示笔记背景颜色
    };
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        // 生成可在整个列表上执行的任何其他操作。在正常安装中，这里没有其他操作，
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // 如果剪贴板上有数据，则启用“粘贴”菜单项。
        ClipboardManager clipboard = (ClipboardManager)
                getSystemService(Context.CLIPBOARD_SERVICE);
        MenuItem mPasteItem = menu.findItem(R.id.menu_paste);
        // 如果剪贴板包含项目，则启用菜单上的“粘贴”选项。
        ...
        if (haveItems) {
            ...
            // 使用一个元素创建意图数组。这将用于根据所选菜单项发送意图。
            Intent[] specifics = new Intent[1];
            // 将数组中的意图设置为对选定注释的URI的编辑操作。
            specifics[0] = new Intent(Intent.ACTION_EDIT, uri);
            // 使用一个元素创建菜单项数组。这将包含编辑选项。
            MenuItem[] items = new MenuItem[1];
            // 使用所选便笺的URI创建不带特定操作的意图。
            ...
            menu.addIntentOptions(
                ...
                // 如果“编辑”菜单项存在，则为其添加快捷方式。
                if (items[0] != null) {
                    // 将“编辑”菜单项快捷方式设置为数字“1”，字母“e”
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.menu_add:   //添加
          /*
           * 使用意图启动新活动。活动的意图筛选器必须插入操作。未设置类别，因此假定为默认类别。
           * 实际上，这将启动记事本中的记事本编辑器活动。
           */
           startActivity(new Intent(Intent.ACTION_INSERT, getIntent().getData()));
           return true;
        case R.id.menu_paste:  //粘贴
          /*
           * 使用意图启动新活动。活动的意图筛选器必须具有操作粘贴。未设置类别，因此假定为默认类别。
           * 实际上，这将启动记事本中的记事本编辑器活动。
           */
          startActivity(new Intent(Intent.ACTION_PASTE, getIntent().getData()));
          return true;
        case R.id.menu_search:  //查找功能
            //startActivity(new Intent(Intent.ACTION_SEARCH, getIntent().getData()));
            Intent intent = new Intent(this, NoteSearch.class);
            this.startActivity(intent);
            return true;
/*
        case R.id.background_color:  //背景颜色
                isFlag=true;
                showColor();
                break;
        case R.id.text_color:  //文字颜色
                isFlag=false;
                showColor();
                break;
